...
export type Question = TrueFalseQuestion | OpenQuestion | MultipleChoiceQuestion
...
interface QuizContextType {
  ...
  // Actions
  createQuiz: (bookId: string, chapterId: string, questions: Question[]) => Promise<void>
...
export function QuizProvider({ children }: QuizProviderProps) {
  ...
  const createQuiz = useCallback(async (bookId: string, chapterId: string, questions: Question[]) => {
    setSaving(true)
    setError(null)

    try {
      const response = await fetch(`http://localhost:8080/api/books/${bookId}/chapters/${chapterId}/quiz`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          questions: questions.map((q) => ({
            ...
            ...(q.type === "TRUE_FALSE" && { isAnswerTrue: (q as TrueFalseQuestion).isAnswerTrue }),
            ...(q.type === "OPEN" && { expectedAnswer: (q as OpenQuestion).expectedAnswer }),
  ...
  const duplicateQuestion = useCallback((index: number) => {
    setQuizState((prev) => {
      if (!prev) return null
      const questionToDuplicate = { ...prev.questions[index], id: crypto.randomUUID() }
      const newQuestions = [...prev.questions]
